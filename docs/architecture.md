...
- 不启动 TUI 也能发现和检查服务。
- macOS、Linux、Windows 以及不同系统数据源可以独立适配。
- 单个进程以及 Git、Docker、HTTP 等 enrichment 的失败不会中断整次扫描；基础监听器扫描完全不可用时明确失败。
...
└── presentation/
    ...
    └── tui/
        ├── app.py              # 应用编排、Worker 和事件处理
        ├── state.py            # 过滤、排序和选中状态
        ├── formatters.py       # 表格、详情和快捷键文案
        ├── screens.py          # 详情、警告、二维码和停止确认弹窗
        ├── widgets.py          # 可复用 Textual 组件
        └── portboard.tcss      # 集中式视觉样式
...
- `ProcessInfo`：PID、创建时间、进程名、命令和工作目录；受权限限制的字段允许为空。创建时间用于危险操作前识别 PID 是否已被复用。
...
3. 应用层合并并规范化基础数据，并以有界线程池并发执行 HTTP 探测。
4. Docker、Git 和 HTTP enrichment 只补充信息，不改变基础发现契约；结果仍按监听器顺序稳定输出。
5. 应用层返回完整的 `ServiceSnapshot`。
6. JSON 输出或 TUI 消费快照；两者不得重复实现发现逻辑。

刷新由展示层触发，默认手动刷新，可通过 `--refresh-seconds` 启用周期刷新。TUI 使用后台 Worker 执行扫描，扫描期间界面和键盘操作保持响应；重复刷新请求合并为最多一个待执行刷新。应用层的单次发现用例不维护永久后台任务，也不保存 UI 状态。
...
- HTTP 返回 4xx/5xx 仍表示端点已识别为 HTTP，可以复制或打开 URL；健康状态单独显示为异常。
- 停止进程必须显示目标 PID、命令和端口并获得明确确认；执行前重新读取并匹配 PID、创建时间、名称、命令和工作目录。缺少创建时间时拒绝执行。
...
- macOS、Linux 和 Windows 上至少各有一条系统扫描集成验证路径。
- 没有 Git 仓库、进程扫描中退出或部分字段无权限时不导致整次失败。
- JSON 输出符合版本 1 契约，且不包含日志或终端控制字符。

### 已实现切片

1. Textual TUI 消费同一个发现用例，并提供手动刷新、可选周期刷新、过滤和排序。
